...
import type { ShuttleDeparture, ShuttleSchedule } from "../services/ShuttleService";
 ...
 */
export const SHUTTLE_STOPS: Record<ShuttleCampus, { latitude: number; longitude: number }> = {
...
function getLastDepartureTime(
    ...
    for (let i = departures.length - 1; i >= 0; i--) {
        ...
        const timeStr = originCampus === "LOY" ? dep.loyola_departure : dep.sgw_departure;
        if (timeStr) {
