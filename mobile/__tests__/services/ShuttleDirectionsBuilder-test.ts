...
import { RouteInfo, DEFAULT_DEPARTURE_CONFIG } from "../../src/types/Directions";
...
const mockFetchDirections =
    ...
    >;
const mockFetchSchedule =
    ShuttleService.fetchSchedule as jest.MockedFunction<
        typeof ShuttleService.fetchSchedule
    >;
const mockFetchRoute =
    ShuttleService.fetchRoute as jest.MockedFunction<
...
describe("ShuttleDirectionsBuilder", () => {
    it("builds a shuttle route with walk + shuttle + walk steps", async () => {
        ...
        expect(result!.distanceMeters).toBeGreaterThan(mockWalkRoute.distanceMeters);
    ...
    it("uses loyola_to_sgw route when origin is LOY", async () => {
        ...
        expect(shuttleStep!.transitDetails!.departureStopName).toBe("Loyola Campus");
        expect(shuttleStep!.transitDetails!.arrivalStopName).toBe("SGW (Hall Building)");
    });
    ...
    it("uses sgw_to_loyola route when origin is SGW", async () => {
        ...
        expect(shuttleStep!.transitDetails!.departureStopName).toBe("SGW (Hall Building)");
